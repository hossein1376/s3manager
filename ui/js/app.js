...
async function loadBuckets(reset = true) {
  ...
  document.querySelector("#load-more-buckets").style.display = bucketNextToken
    ? "block"
    : "none";
  ...
  document.querySelector("#load-more-buckets").onclick = () =>
    loadBuckets(false);
}
...
function openBucket(name, path) {
  window.location.href = `objects.html?bucket=${encodeURIComponent(name)}&path=${encodeURIComponent(path)}`;
}
...
async function loadObjects(reset = true) {
  ...
  const url = new URL(
    `${API_BASE}/buckets/${bucket}?path=${encodeURIComponent(path)}`,
  );
  ...
  document.querySelector("#load-more-objects").style.display = objectNextToken
    ? "block"
    : "none";
  ...
  document.querySelector("#load-more-objects").onclick = () =>
    loadObjects(false);
  ...
  document.getElementById("delete-selected").onclick = async () => {
    const keys = [...document.querySelectorAll(".select-object:checked")].map(
      (cb) => cb.value,
    );
...
async function deleteObject(bucket, key) {
  ...
  const res = await fetch(
    `${API_BASE}/buckets/${bucket}/objects/${encodeURIComponent(key)}`,
    { method: "DELETE" },
  );
  if (!res.ok) {
